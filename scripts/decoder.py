...
class Decoder():
    def __init__(self, model, tokenizer, init_method, iter_method, MAX_ITER, BEAM_SIZE, NUM_MASK, BATCH_SIZE):
        ...
        for b_out_tensor, b_logprob, b_mask_ind in zip(out_tensor, logprob, mask_ind):
            for i in range(self.NUM_MASK):
                ...
                for j in range(self.BEAM_SIZE):
                    ...
                    lp = np.power(mask_len,length_norm_coeff)
    ...
    def decode(self, input, batch_size=None):
        """
        ...
        all_preds = []
        all_probs = []
        for b in tqdm(range(0, len(input), batch_size),desc="decode"):
